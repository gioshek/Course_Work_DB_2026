...
const SQL_OBJECT_EXPLANATIONS = {
  vw_BookCatalog: {
    summary: "Каталог книг: собирает данные книги, авторов, жанров, рейтинга, скидки и итоговой цены.",
    details: `Это главное представление каталога. Оно объединяет таблицы Book, Publisher, BookAuthor, Author, BookGenre, Genre, Review, Promotion и BookPromotion. Благодаря этому backend получает уже готовую строку книги: название, описание, авторов, жанры, издательство, рейтинг, количество отзывов, цену, процент скидки и итоговую цену. Используется в каталоге, карточке книги, поиске и отчётах администратора.`,
  },
  vw_BookReviews: {
    summary: "Отзывы по книгам с пользователями и названиями произведений.",
    details: `Представление соединяет Review, Book и UserAccount. Оно нужно, чтобы получать не просто ReviewId и UserId, а понятную информацию для интерфейса: название книги, имя пользователя, оценку, текст отзыва и дату создания. Используется процедурой получения карточки книги и блоком отзывов на странице произведения.`,
  },
  vw_PopularBooks: {
    summary: "Популярные книги по покупкам, избранному, отзывам и рейтингу.",
    details: `Представление считает агрегированную популярность книги: количество покупок, добавлений в избранное, отзывов и средний рейтинг. Оно показывает, какие книги активнее всего используются пользователями. Используется в админской статистике и во вкладке отчётов как готовый аналитический объект БД.`,
  },
  vw_UserLibrary: {
    summary: "Библиотека пользователя: купленные, бесплатные и доступные по подписке книги.",
    details: `Представление формирует список книг, к которым пользователь имеет доступ. В него попадают купленные книги, бесплатные книги и книги, доступные по активной подписке. Используется в разделе «Моя библиотека», чтобы не собирать доступ вручную на frontend.`,
  },
  vw_UserPayments: {
    summary: "История платежей пользователей с суммой, способом оплаты и статусом.",
    details: `Представление подготавливает историю платежей: пополнения баланса, покупки и подписки. Оно связывает Payment с UserAccount, чтобы в профиле и отчётах показывать имя пользователя, сумму, статус, метод оплаты, номер транзакции и дату. Используется в личном кабинете и административной отчётности.`,
  },
  vw_UserReadingProgress: {
    summary: "Прогресс чтения пользователей по книгам.",
    details: `Представление объединяет ReadingProgress, Book и UserAccount. Оно показывает, какую книгу читает пользователь, текущую страницу, общее число страниц и процент прогресса. Используется в профиле пользователя и демонстрирует работу отдельной сущности ReadingProgress.`,
  },
  vw_ActiveBookPromotions: {
    summary: "Активные акции, которые сейчас действуют на книги.",
    details: `Представление выбирает только активные акции из Promotion и BookPromotion, у которых текущая дата входит в период действия. Оно показывает, на какие книги сейчас действует скидка, какой промокод используется и какой процент скидки применён. Используется для проверки работы акций и для административной панели.`,
  },

  usp_GetBookCatalog: {
    summary: "Получает каталог книг с фильтрами по поиску, жанру, бесплатности и подписке.",
    details: `Хранимая процедура возвращает список книг на основе vw_BookCatalog. Она умеет фильтровать каталог по тексту, жанру, признаку бесплатной книги и доступности по подписке. В backend дополнительно применяется нечёткий поиск через Python, но основная выдача книг всё равно берётся из SQL-процедуры.`,
  },
  usp_GetBookById: {
    summary: "Получает карточку книги и список отзывов.",
    details: `Процедура используется при открытии страницы конкретной книги. Первым набором данных она возвращает информацию о книге из vw_BookCatalog, включая скидку и итоговую цену. Вторым набором данных возвращает отзывы из vw_BookReviews.`,
  },
  usp_GetBookContentForUser: {
    summary: "Открывает текст книги только при наличии доступа у пользователя.",
    details: `Процедура проверяет, может ли пользователь читать книгу: книга бесплатная, куплена пользователем или доступна по активной подписке. Если доступ есть, возвращается текст из BookContent и прогресс чтения. Если доступа нет, backend получает ошибку и не показывает текст книги.`,
  },
  usp_UpdateReadingProgress: {
    summary: "Сохраняет текущую страницу и процент прочтения.",
    details: `Процедура обновляет или создаёт запись в ReadingProgress. На основе текущей страницы и общего количества страниц книги рассчитывается процент чтения. Используется в читалке, когда пользователь сохраняет прогресс.`,
  },
  usp_AddReview: {
    summary: "Добавляет или обновляет отзыв пользователя к книге.",
    details: `Процедура сохраняет оценку и текст отзыва в таблицу Review. Она нужна, чтобы пользователь мог оценить книгу, а каталог и отчёты могли учитывать средний рейтинг и количество отзывов. После добавления отзыва триггер записывает действие в AuditLog.`,
  },
  usp_BuyBook: {
    summary: "Покупает книгу с проверкой баланса и применением скидки.",
    details: `Одна из ключевых процедур проекта. Она проверяет существование пользователя и книги, смотрит, не была ли книга куплена раньше, вычисляет активную скидку через функции fn_GetBookActiveDiscountPercent и fn_GetBookFinalPrice, проверяет баланс, списывает итоговую цену, создаёт Payment и Purchase. Используется backend-операцией покупки книги.`,
  },
  usp_CreateSubscription: {
    summary: "Оформляет подписку пользователю с оплатой с баланса.",
    details: `Процедура создаёт подписку пользователя на выбранный тариф из SubscriptionPlan. Она проверяет баланс, списывает стоимость подписки, создаёт платёж и запись в UserSubscription. Используется на странице подписок и демонстрирует связь подписок, платежей и пользователей.`,
  },
  usp_GetUserLibrary: {
    summary: "Возвращает книги, доступные пользователю в библиотеке.",
    details: `Процедура формирует библиотеку пользователя: бесплатные книги, купленные книги и книги по активной подписке. Она также возвращает CoverImageUrl, чтобы в «Моей библиотеке» отображались обложки. Используется endpoint-ом библиотеки пользователя.`,
  },
  usp_GetUserProfile: {
    summary: "Собирает данные личного кабинета пользователя.",
    details: `Процедура возвращает профиль пользователя, активные подписки, платежи и прогресс чтения. Благодаря ей личный кабинет получает данные из нескольких сущностей БД одним запросом. Используется страницей профиля.`,
  },
  usp_TopUpUserBalance: {
    summary: "Пополняет баланс пользователя и создаёт платёж.",
    details: `Процедура проверяет корректность суммы пополнения, увеличивает Balance в UserAccount и создаёт запись в Payment. Используется в личном кабинете при пополнении баланса. Через неё задействуется финансовая часть проекта.`,
  },
  usp_GetPromotions: {
    summary: "Возвращает список акций и книги, привязанные к ним.",
    details: `Процедура используется админкой во вкладке «Акции». Первый результат — список Promotion с количеством книг в каждой акции. Второй результат — связи BookPromotion с названиями книг, базовой и итоговой ценой. Это позволяет администратору управлять скидками через интерфейс сайта.`,
  },
  usp_CreatePromotion: {
    summary: "Создаёт или обновляет акцию по промокоду.",
    details: `Процедура проверяет название акции, промокод, процент скидки и даты действия. Если акция с таким промокодом уже есть, она обновляется; если нет — создаётся новая запись в Promotion. Используется администратором для управления скидками.`,
  },
  usp_AssignPromotionToBook: {
    summary: "Привязывает выбранную книгу к акции.",
    details: `Процедура создаёт связь между Promotion и Book в таблице BookPromotion. Она проверяет существование акции и книги, а затем добавляет связь, если её ещё нет. После этого скидка автоматически начинает учитываться в каталоге и при покупке.`,
  },
  usp_RemovePromotionFromBook: {
    summary: "Убирает конкретную книгу из акции.",
    details: `Процедура удаляет одну связь из BookPromotion, не удаляя саму акцию. Она нужна, когда администратор хочет оставить акцию, но исключить из неё отдельную книгу. Действие фиксируется триггером BookPromotion в AuditLog.`,
  },
  usp_DeletePromotion: {
    summary: "Полностью удаляет акцию и все её связи с книгами.",
    details: `Процедура удаляет запись из Promotion. Так как связь BookPromotion настроена с каскадным удалением, все книги автоматически отвязываются от этой акции. Это нормальная операция администратора, когда акция больше не нужна.`,
  },
  usp_AdminSalesReport: {
    summary: "Формирует отчёт продаж за период с группировкой.",
    details: `Интерактивная отчётная процедура. Администратор выбирает период и тип группировки: по книгам, пользователям или дням. Процедура выполняет агрегирующие запросы по Purchase и связанным таблицам, возвращая количество покупок, сумму продаж и среднюю цену.`,
  },
  usp_AdminBookReport: {
    summary: "Формирует отчёт по книгам с фильтрами.",
    details: `Процедура позволяет анализировать книги по жанру, издательству, минимальному рейтингу и наличию скидки. Она использует каталог, покупки, отзывы и акции, чтобы показать цену, скидку, итоговую цену, рейтинг, покупки и отзывы. Используется во вкладке интерактивных отчётов.`,
  },
  usp_AdminUserReport: {
    summary: "Формирует отчёт по пользователям и их активности.",
    details: `Процедура анализирует пользователей: статус активности, дату регистрации, баланс, сумму покупок, количество покупок, отзывов и избранных книг. Администратор может отфильтровать пользователей по активности, датам регистрации и минимальной сумме покупок.`,
  },
  usp_AdminGenreReport: {
    summary: "Формирует отчёт по жанрам, продажам и рейтингам.",
    details: `Процедура группирует данные по жанрам. Она показывает количество книг жанра, количество покупок, сумму продаж, число отзывов и средний рейтинг. Используется, чтобы понять, какие жанры приносят больше продаж и активности.`,
  },
  usp_AdminAuditLogReport: {
    summary: "Формирует отчёт по журналу действий AuditLog.",
    details: `Процедура позволяет фильтровать AuditLog по таблице, действию и периоду. Это удобный интерфейс для проверки триггеров: можно показать, какие INSERT, UPDATE и DELETE были автоматически зафиксированы базой данных.`,
  },

  fn_GetBookAverageRating: {
    summary: "Возвращает средний рейтинг выбранной книги.",
    details: `Скалярная функция считает среднее значение Rating из Review для одной книги. Она полезна для каталога, карточки книги и отчётов, где нужно быстро получить агрегированную оценку произведения. Если отзывов нет, функция возвращает нейтральное значение по логике SQL-кода.`,
  },
  fn_GetBookReviewCount: {
    summary: "Возвращает количество отзывов по выбранной книге.",
    details: `Скалярная функция считает число записей Review для конкретной книги. Используется для отображения активности вокруг книги и для аналитики администратора. Показывает, насколько активно пользователи оценивают и обсуждают произведение.`,
  },
  fn_UserHasAccessToBook: {
    summary: "Проверяет право пользователя на чтение книги.",
    details: `Функция возвращает признак доступа к книге. Она учитывает бесплатные книги, покупки пользователя и активную подписку. Используется процедурами чтения и библиотекой, чтобы централизованно решать, можно ли открыть текст книги.`,
  },
  fn_GetUserBalance: {
    summary: "Возвращает текущий баланс пользователя.",
    details: `Скалярная функция получает Balance из UserAccount. Она нужна для финансовой логики: профиль, покупки, подписки и проверки достаточности средств. В проекте баланс используется как внутренняя платёжная система.`,
  },
  fn_GetBookActiveDiscountPercent: {
    summary: "Возвращает максимальную активную скидку книги на дату.",
    details: `Функция ищет активные Promotion, связанные с книгой через BookPromotion, проверяет даты действия и берёт максимальный DiscountPercent. Используется при показе каталога и при покупке, чтобы скидка считалась одинаково в интерфейсе и в финансовой операции.`,
  },
  fn_GetBookFinalPrice: {
    summary: "Рассчитывает итоговую цену книги с учётом скидки.",
    details: `Функция берёт базовую цену книги и активную скидку, затем рассчитывает финальную цену. Бесплатные книги получают цену 0. Используется каталогом, страницей книги, акциями и процедурой покупки, чтобы пользователь платил именно скидочную цену.`,
  },
  fn_GetBooksByGenre: {
    summary: "Возвращает книги выбранного жанра.",
    details: `Табличная функция возвращает книги, связанные с указанным жанром. Она демонстрирует использование пользовательской табличной функции в БД и может применяться для жанровой фильтрации или отчётов.`,
  },
  fn_GetAccessibleBooksForUser: {
    summary: "Возвращает книги, доступные конкретному пользователю.",
    details: `Табличная функция формирует набор книг, которые пользователь может читать. Она учитывает бесплатность, покупки и активную подписку. По смыслу используется для библиотеки и проверки доступа к контенту.`,
  },
  fn_UserHasActiveSubscription: {
    summary: "Проверяет наличие активной подписки у пользователя.",
    details: `Скалярная функция проверяет, есть ли у пользователя подписка, которая активна на выбранную дату. Она используется при определении доступа к книгам по подписке и помогает не дублировать одну и ту же проверку в разных SQL-запросах.`,
  },

  trg_UserAccount_AfterInsert: {
    summary: "Логирует регистрацию нового пользователя.",
    details: `Триггер срабатывает после вставки записи в UserAccount. Он добавляет запись в AuditLog, чтобы администратор видел факт регистрации пользователя. Это демонстрирует автоматическое журналирование действий на уровне БД.`,
  },
  trg_Book_AfterInsert: {
    summary: "Логирует добавление новой книги.",
    details: `Триггер срабатывает после добавления книги в Book. Он записывает событие в AuditLog, поэтому создание книги через админку автоматически фиксируется базой данных без отдельного кода на frontend.`,
  },
  trg_Review_AfterInsert: {
    summary: "Логирует добавление отзыва.",
    details: `Триггер срабатывает при вставке Review. Он фиксирует в AuditLog, что пользователь оставил отзыв к книге. Это помогает отслеживать пользовательскую активность и работу сущности Review.`,
  },
  trg_Purchase_AfterInsert: {
    summary: "Логирует покупку книги.",
    details: `Триггер срабатывает после создания Purchase. Он записывает факт покупки в AuditLog. Благодаря этому администратор видит не только финансовую запись, но и бизнес-событие покупки книги.`,
  },
  trg_Payment_AfterInsert: {
    summary: "Логирует создание платежа.",
    details: `Триггер срабатывает после вставки Payment. Он фиксирует пополнения баланса, оплату покупок и подписок. Используется для демонстрации автоматического аудита финансовых операций.`,
  },
  trg_UserSubscription_AfterInsert: {
    summary: "Логирует оформление подписки.",
    details: `Триггер срабатывает при создании UserSubscription. Он фиксирует, что пользователь оформил подписку на тариф. Это связывает подписочную модель проекта с журналом действий.`,
  },
  trg_ReadingProgress_AfterInsertUpdate: {
    summary: "Логирует создание и обновление прогресса чтения.",
    details: `Триггер срабатывает при вставке или обновлении ReadingProgress. Он показывает, что даже действия внутри читалки отражаются в БД. Это полезно для демонстрации того, что прогресс чтения является полноценной сущностью проекта.`,
  },
  trg_Promotion_AfterInsertUpdate: {
    summary: "Логирует создание и изменение акций.",
    details: `Триггер срабатывает после INSERT или UPDATE в Promotion. Он фиксирует создание акции, изменение процента скидки, периода действия или активности. Используется во вкладке «Журнал» для проверки работы административных операций со скидками.`,
  },
  trg_BookPromotion_AfterInsertDelete: {
    summary: "Логирует привязку и отвязку книг от акций.",
    details: `Триггер срабатывает после добавления или удаления связи в BookPromotion. Он показывает, какие книги были включены в акцию или исключены из неё. Это важно, потому что скидки в проекте работают через связь многие-ко-многим между книгами и акциями.`,
  },
};
            ...
            return (
              <div key={promotion.PromotionId} className="promotion-card">
                ...
                <div className="details-meta">
                  ...
                  <span>Книг: {promotion.BookCount}</span>
                </div>
        ...
        <section className="panel database-dashboard-panel admin-tab-panel">
          <div className="section-heading">
            <div>
              ...
              <p className="muted">
                Здесь администратор сам задаёт параметры, а backend вызывает SQL-процедуры из файла 12_admin_reports.sql.
              </p>
            </div>
